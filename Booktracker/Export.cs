...
public static class Export
{
    ...
    //             new DB().BookPageInfo book = listOfBooks[i];
    //             string row = "";
    ...
    public static void ExportDataAsCSV(List<DB.BookPageInfo> listOfBooks, string username)
    {
        ...
        {
            string columns =
                "id,bookID,title,author,publishedDate,publisher,imageLink,status,rating,dateStarted,dateFinished,description,pageCount,isbn,category";
            writer.WriteLine(columns);
            ...
            for (int i = 0; i < listOfBooks.Count; i++)
            {
                ...
                row +=
                    $"\"{EscapeField(book.Title)}\",\"{EscapeField(book.Author)}\",\"{EscapeField(book.PublishedDate)}\",\"{EscapeField(book.Publisher)}\",\"{EscapeField(book.ImageLink)}\",";
                row +=
                    $"\"{EscapeField(book.Status)}\",\"{EscapeField(book.Rating)}\",\"{EscapeField(book.DateStarted)}\",\"{EscapeField(book.DateFinished)}\",\"{EscapeField(book.Description)}\",\"{EscapeField(book.PageCount)}\",\"{EscapeField(book.Isbn)}\",\"{EscapeField(book.Category)}\"";
                writer.WriteLine(row);
            }
        }
    }
