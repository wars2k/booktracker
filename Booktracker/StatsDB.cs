...
public class StatsDB
{
    ...
    public StatsTypes.StatusCounts GetStatusCounts(SessionInfo session)
    {
        string sql =
            "SELECT COUNT(CASE WHEN iduser = @id AND status = 'UNASSIGNED')AS count_unassigned, COUNT(CASE WHEN iduser = @id AND status = 'READING')AS count_reading, COUNT(CASE WHEN iduser = @id AND status = 'UP NEXT')AS count_next, COUNT(CASE WHEN iduser = @id AND status = 'WISHLIST')AS count_wishlist, COUNT(CASE WHEN iduser = @id AND status = 'FINISHED')AS count_finished FROM user_books";
